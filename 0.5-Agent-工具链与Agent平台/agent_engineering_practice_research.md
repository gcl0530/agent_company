...
3. **人类合并与发布门**：高影响路径需要明确 owner 审查；任何 Agent 都不绕过对外承诺、合并、发布、验收与风险接受。

### 按需试用的扩展

1. **交付切片**：前端、后端、算法和集成同时参与同一个结果时，用可验证的端到端结果组织协作；单模块项目可不启用。
2. **契约与评测资产**：跨模块变更才要求 owner、兼容性、失败语义和可版本化的正常/边界/失败样本。
3. **运行可审计**：对 M1 及以上、需要复用或扩大权限的工作单元记录输入、动作范围、工具事实、成本、复核和异常；M0 草案无需强制运行记录。
